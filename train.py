...
def train(args):
    ...
    if use_cuda:
        model.cuda()
        global_model.cuda()
        total_data = total_data.cuda() #added eval_paper_authors
    ...
    for epoch in range(args.max_epochs):
        ...
        t3 = time.time()
        print("Epoch {:04d} | Loss {:.4f} | time {:.4f}". format(epoch, loss_epoch / (len(train_data) / args.batch_size), t3 - t0))
        ...
        if epoch % args.valid_every == 0 and epoch >= int(args.max_epochs / 2):
            ...
            for i in range(len(valid_data)):
                ...
                with torch.no_grad():
                    ranks, loss = model.evaluate_filter(batch_data, (s_hist, s_hist_t), (o_hist, o_hist_t),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--setting', type=str, default='raw', choices=['raw', 'static', 'time'],
                        help="filtering setting") #added eval_paper_authors for logging
