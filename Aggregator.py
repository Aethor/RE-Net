...
class RGCNAggregator_global(nn.Module):
    ...
    def __init__(self, h_dim, dropout, num_nodes, num_rels, num_bases, model, seq_len=10, maxpool=1, use_cuda=True): #modified eval_paper_authors: use_cuda request
        ...
        self.use_cuda = use_cuda # added eval_paper_authors

    def forward(self, t_list, ent_embeds, graph_dict, reverse):
        if self.use_cuda: #modified eval_paper_authors: use_cuda request
            ...
            for tim in unique_t:
                ...
                idx += 1
                g_list.append(graph_dict[tim.item()])
            g_list = [g.to(torch.device('cpu')) for g in g_list]  # added eval_paper_authorspaper_authors
            batched_graph = dgl.batch(g_list)
            batched_graph.ndata['h'] = ent_embeds[batched_graph.ndata['id']].view(-1, ent_embeds.shape[1])
            # move_dgl_to_cuda(batched_graph)
            self.rgcn1(batched_graph, reverse)
            ...
            embed_seq_tensor = torch.zeros(len(len_non_zero), self.seq_len, self.h_dim)
            for i, times in enumerate(time_list):
                for j, t in enumerate(times):
                    embed_seq_tensor[i, j, :] = global_info[time_to_idx[t.item()]]
    ...
    def predict(self, t, ent_embeds, graph_dict, reverse):
        if self.use_cuda: #modified eval_paper_authorspaper_authorspaper_authors: use_cuda request
            times = list(graph_dict.keys())
            ...
            if self.maxpool == 1:
                ...
                global_info = dgl.mean_nodes(batched_graph, 'h')
            batched_graph.ndata.pop('h')
        else: #added by eval_paper_authorspaper_authorspaper_authors, to able to run without cuda.
            times = list(graph_dict.keys())
            ...
            for tim in timess:
                graph_dict[tim.item()]
                # print(graph_dict[tim.item()])
                # graph_dict[tim.item()].cpu()
                g_list.append(graph_dict[tim.item()]) #modified eval_paper_authorspaper_authorspaper_authors
...
class RGCNAggregator(nn.Module):
    ...
    def __init__(self, h_dim, dropout, num_nodes, num_rels, num_bases, model, seq_len=10, use_cuda=True): #eval_paper_authorspaper_authorspaper_authorspaper_authors added use_cuda
        ...
        self.rgcn2 = RGCNLayer(self.h_dim, self.h_dim, 2*self.num_rels, num_bases,
                               activation=None, self_loop=True, dropout=dropout)
        self.use_cuda = use_cuda # modified eval_paper_authorspaper_authorspaper_authors: added 
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds, graph_dict, global_emb, reverse):
        ...
        if length == 0:
            ...
            s_len_non_zero, s_tem, r_tem, g, node_ids_graph, global_emb_list = get_sorted_s_r_embed_rgcn(s_hist, s, r,
                                                                                                         ...
                                                                                                         graph_dict,
                                                                                                         global_emb,
                                                                                                         self.use_cuda) # modified eval_paper_authors: added use_cuda
            if g is None:
                s_packed_input = None
            else:

                if self.use_cuda: #modified eval_paper_authorspaper_authors: use_cuda request
                    ...
                    embeds_split = torch.split(embeds_mean, s_len_non_zero.tolist())
                    global_emb_list_split = torch.split(global_emb_list, s_len_non_zero.tolist())
                    s_embed_seq_tensor = torch.zeros(len(s_len_non_zero), self.seq_len, 4 * self.h_dim)

                    s_embed_seq_tensor_r = torch.zeros(len(s_len_non_zero), self.seq_len, 3 * self.h_dim)
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds, graph_dict, global_emb, reverse):
        ...
        if self.use_cuda: #modified eval_paper_authors: use_cuda request
            ...
            inp = torch.zeros(len(s_hist), 4 * self.h_dim)
            inp[torch.arange(len(embeds)), :] = torch.cat(
                (embeds, ent_embeds[s].repeat(len(embeds), 1), rel_embeds[r].repeat(len(embeds), 1), global_emb_list),
                dim=1)

            inp_r = torch.zeros(len(s_hist), 3 * self.h_dim)
            inp_r[torch.arange(len(embeds)), :] = torch.cat((embeds, ent_embeds[s].repeat(len(embeds), 1), global_emb_list),
                                                            dim=1)
        return inp, inp_r
...
class MeanAggregator(nn.Module):
    ...
    def __init__(self, h_dim, dropout, seq_len=10, gcn=False, use_cuda=True):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds):
        s_len_non_zero, s_tem, r_tem, embeds_stack, len_s, embeds_split = get_sorted_s_r_embed(s_hist, s, r, ent_embeds, self.use_cuda)  #modified eval_paper_authors: use_cuda r
        if self.use_cuda: #modified eval_paper_authors: use_cuda request
            ...
            embeds_sum = torch.sparse.mm(mask_tensor, embeds_stack)
            embeds_mean = embeds_sum / torch.Tensor(len_s).view(-1, 1)

            if self.gcn:
                embeds_mean = self.gcn_layer(embeds_mean)
                embeds_mean = F.relu(embeds_mean)
            embeds_split = torch.split(embeds_mean, s_len_non_zero.tolist())
            s_embed_seq_tensor = torch.zeros(len(s_len_non_zero), self.seq_len, 2 * self.h_dim)
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds):
        if self.use_cuda: #modified eval_paper_authors: use_cuda request
            ...
            inp = torch.zeros(len(s_history), 2 * self.h_dim)
...
class AttnAggregator(nn.Module):
    def __init__(self, h_dim, dropout, seq_len=10, use_cuda=True):   #modified eval_paper_authors: added use_cuda
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds):
        ...
        s_len_non_zero, s_tem, r_tem, embeds_stack, len_s, embeds_split = get_sorted_s_r_embed(s_hist, s, r, ent_embeds, self.use_cuda)  #modified eval_paper_authors: added use_cuda
        if self.use_cuda: #modified eval_paper_authors: use_cuda request
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds):
        ...
        else:   #modified eval_paper_authors: added use_cuda
            inp = torch.zeros(len(s_history), 3 * self.h_dim)
